...
function TabsList({
	...
	return (
		<TabsPrimitive.List
			className={cn(
				`text-muted-foreground relative z-0 flex w-fit items-center
		...
		>
			...
			<TabsPrimitive.Indicator
				className={cn(
					...
					variant === 'underline'
						? `bg-primary z-10 data-[orientation=horizontal]:h-0.5
...
function TabsTab({ className, ...props }: TabsPrimitive.Tab.Props) {
	return (
		<TabsPrimitive.Tab
			className={cn(
				`focus-visible:ring-ring flex shrink-0 grow cursor-pointer items-center
				justify-center rounded-md border border-transparent text-base
				font-medium whitespace-nowrap
				transition-[color,background-color,box-shadow] outline-none
				focus-visible:ring-2 data-disabled:pointer-events-none
				data-disabled:opacity-64 sm:text-sm [&_svg]:pointer-events-none
